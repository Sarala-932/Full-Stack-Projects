...
export default function BudgetProgress({accountId, initialBudget, currentExpenses, categoryExpenses, onUpdate}) {
  ...
  const rawPercentage = initialBudget && initialBudget.amount > 0
  ...
  const expenseCategories = defaultCategories
    .filter(c => c.type === "EXPENSE" && !categoryLimits.some(cl => cl.category === c.id));
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {categoryLimits.length === 0 && !isAddingCategory ? (
              <p className="text-sm text-muted-foreground text-center py-4">No category limits set.</p>
            ) : (
                ...
                return (
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium">{categoryInfo?.name || limit.category}</span>
                        {editingCategory === limit.category ? (
                          <div className="flex items-center gap-2 ml-2">
                            <Input
                              type="number"
                              value={editingCategoryAmount}
                              onChange={(e) => setEditingCategoryAmount(e.target.value)}
                    ...
                    <Progress
                      value={catPercent}
                      extraStyles={`${
                        catPercent >= 90 ? "bg-red-500" : catPercent >= 75 ? "bg-yellow-500" : "bg-primary"
                      }`}
                      className="h-2"
                    />
                  </div>
